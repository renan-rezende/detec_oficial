...
Sistema industrial em Python para detecção, medição e classificação granulométrica de pelotas de minério em tempo real, usando segmentação por instância com YOLO e aceleração TensorRT/ONNX em GPU NVIDIA.
...
│   ├── camera_manager.py     # Orquestração multi-câmera com threads
│   └── csv_logger.py         # Logger CSV thread-safe por câmera
│
...
│
├── models/                   # Modelos pre-treinados (não versionados em git)
...
| `model_path` | str | `RGB_960m_256.engine` | Caminho para modelo .engine ou .onnx |
...
- `range_*`: Proporção em área de cada faixa (0.0 a 1.0, soma = 1.0) — calculada como `soma_area_mm²[faixa] / área_total_mm²`, refletindo o peso relativo de cada faixa como no ensaio de laboratório
...
O modelo `.engine` é específico por GPU e deve ser gerado na máquina de destino:
...
- Interface Tkinter congela ao arrastar janela no Windows (comportamento normal do framework; processamento backend continua).
- Escala deve ser calibrada individualmente por câmera/ângulo de câmera.
...
| Pacote | Uso |
|--------|-----|
| `ultralytics` | YOLO (inferência, exportação) |
...
| `psutil` | Informações de sistema |

## Documentação Complementar

- [ARCHITECTURE.md](ARCHITECTURE.md) — Arquitetura, fluxo de dados, modelo de threads
